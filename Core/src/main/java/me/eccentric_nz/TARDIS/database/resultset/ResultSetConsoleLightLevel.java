 ...
 * @author eccentric_nz
 */
public class ResultSetConsoleLightLevel {
     ...
     */
    public ResultSetConsoleLightLevel(TARDIS plugin, int id) {
        this.plugin = plugin;
        this.id = id;
    ...
    public boolean resultSet() {
        ...
        ResultSet rs = null;
        String query = "SELECT secondary FROM " + prefix + "controls WHERE `type` = 56 AND `tardis_id` = ?";
        try {
            ...
            plugin.debug("ResultSet error for console light level control from id! " + e.getMessage());
            return false;
        } finally {
            try {
                if (rs != null) {
                ...
                plugin.debug("Error closing console light level control from id! " + e.getMessage());
            }
        }
        return true;
    }
