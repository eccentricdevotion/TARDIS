 ...
 * @author eccentric_nz
 */
public class ResultSetOuterDoorBlock {
     ...
     */
    public ResultSetOuterDoorBlock(TARDIS plugin, int id) {
        this.plugin = plugin;
        this.id = id;
    ...
    public boolean resultSet() {
        ...
        ResultSet rs = null;
        String query = "SELECT * FROM " + prefix + "doors WHERE door_type = 0 AND tardis_id = ?";
        try {
            ...
            if (rs.isBeforeFirst()) {
                while (rs.next()) {
                    // get block
                    Location doorLocation = TARDISStaticLocationGetters.getLocationFromDB(rs.getString("door_location"));
