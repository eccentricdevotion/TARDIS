...
public final class DramaticBiomeGenerator {
    ...
    private DramaticBiomeGenerator() {
    }

    public static void generate(WorldInfo worldInfo, ChunkData chunkData,
                                int chunkX, int chunkZ, NoiseSet noise) {
        ...
        // Second pass for structures whose shape depends on the finished
        // chamber ceiling/floor.
        addLargeFeatures(worldInfo, chunkData, noise, chunkX, chunkZ, minY, maxY);
    }

    private static void carveColumn(ChunkData chunkData, NoiseSet noise, BiomeStyle style,
                                    ChamberShape shape, int wx, int wz, int x, int z,
                                    int minY, int maxY) {
        BiomeProfile profile = BiomeStyle.getProfile(style);

        double local3d = noise.chamberNoise().noise(
                wx / 42.0, shape.centerY() / 30.0, wz / 42.0);

        // The broad field controls the existence of the chamber; the 3D field
        // only roughens the edge.
    ...
    private static void addLargeFeatures(WorldInfo worldInfo, ChunkData chunkData,
                                         NoiseSet noise, int chunkX, int chunkZ,
                                         int minY, int maxY) {
        int originX = chunkX << 4;
        int originZ = chunkZ << 4;
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                int wx = originX + x;
                int wz = originZ + z;
                BiomeStyle style = BiomeStyle.fromBiome(
                        worldInfo.vanillaBiomeProvider().getBiome(worldInfo, wx, 64, wz));
                ...
                if (style == BiomeStyle.JUNGLE || style == BiomeStyle.SWAMP) {
                    addRoots(chunkData, noise, style, x, z, wx, wz, minY, maxY);
                }
                ...
                if (style == BiomeStyle.FROZEN || style == BiomeStyle.SNOWY
    ...
    private static void addRoots(ChunkData chunkData, NoiseSet noise, BiomeStyle style,
                                 int x, int z, int wx, int wz, int minY, int maxY) {
        ...
        for (int i = 1; i <= length; i++) {
            ...
            if (i > 2 && i % 3 == 0) {
                double branch = noise.rootNoise().noise(
                        (wx + i * 7) / 15.0, (wz - i * 5) / 15.0);
                if (branch > 0.35) {
    ...
    private static void addOceanDetails(ChunkData chunkData, NoiseSet noise,
                                        int x, int z, int wx, int wz,
                                        int minY, int maxY) {
        ...
        if (shelf > 0.35 && floor + 1 <= maxY
                && chunkData.getType(x, floor + 1, z) == Material.WATER) {
        ...
        if (noise.floodNoise().noise(wx / 55.0, wz / 55.0) > 0.72
                && floor + 1 < maxY
                && chunkData.getType(x, floor + 1, z) == Material.WATER) {
    ...
    private static void addMushroomDetails(ChunkData chunkData, NoiseSet noise,
                                           int x, int z, int wx, int wz,
                                           int minY, int maxY) {
        int floor = findFloor(chunkData, x, z, minY, maxY);
        if (floor < minY || floor + 1 >= maxY
                || chunkData.getType(x, floor + 1, z) != Material.AIR) {
            return;
        }
        ...
        if (n > 0.50) {
            ...
            if (noise.rootNoise().noise(wx / 8.0, wz / 8.0) > 0.45) {
                chunkData.setBlock(x, floor + 1, z,
                        noise.rootNoise().noise(wx / 5.0, wz / 5.0) > 0
                                ? Material.RED_MUSHROOM : Material.BROWN_MUSHROOM);
            }
        }
    }

    private static void addIceDetails(ChunkData chunkData, NoiseSet noise,
                                      int x, int z, int wx, int wz,
                                      int minY, int maxY) {
        int ceiling = findCeiling(chunkData, x, z, maxY);
        if (ceiling < 45 || ceiling > 115) {
    ...
    private static int findWaterFloor(ChunkData chunkData, int x, int z,
                                      int minY, int maxY) {
    ...
    private static int findFloor(ChunkData chunkData, int x, int z,
                                 int minY, int maxY) {
    ...
    private static boolean isDramatic(BiomeStyle style) {
        return switch (style) {
            case JUNGLE, OCEAN, COLD_OCEAN, WARM_OCEAN, SWAMP,
                 MUSHROOM, FROZEN, SNOWY, TAIGA, FOREST, LUSH,
                 DESERT, BADLANDS, SCULK, CHERRY, PALE, DRIPSTONE -> true;
            default -> false;
        };
    }
    ...
    private static double dramaticThreshold(BiomeStyle style) {
        return switch (style) {
            // Oceans are intentionally common because their regional identity
            // is the flooded megacavern itself.
    ...
    private static ChamberShape shapeFor(BiomeStyle style, double strength, double flood) {
        double variation = Math.max(0.0, Math.min(1.0, (strength + 1.0) * 0.5));
        double drift = flood * 0.45;

        return switch (style) {
            // Very large vertical space: intentionally compatible with tall
            // trees produced later by CaveTreePopulator.
            case JUNGLE -> new ChamberShape(77 + flood * 8,
                    31 + variation * 10, false, 0, drift, strength);

            case OCEAN, COLD_OCEAN, WARM_OCEAN -> new ChamberShape(
                    66 + flood * 7, 34 + variation * 12,
                    true, 66 + flood * 6, drift, strength);

            case SWAMP -> new ChamberShape(61 + flood * 6,
                    26 + variation * 8, true, 57 + flood * 5, drift, strength);

            case MUSHROOM -> new ChamberShape(76,
                    35 + variation * 10, false, 0, drift, strength);

            case FROZEN, SNOWY -> new ChamberShape(72,
                    31 + variation * 9, false, 0, drift, strength);

            case SCULK -> new ChamberShape(54,
                    28 + variation * 9, false, 0, drift, strength);

            case DESERT, BADLANDS -> new ChamberShape(69 + flood * 5,
                    24 + variation * 7, false, 0, drift, strength);

            case TAIGA, FOREST, LUSH, CHERRY, PALE, DRIPSTONE -> new ChamberShape(
                    75 + flood * 5, 27 + variation * 9,
                    false, 0, drift, strength);

            default -> new ChamberShape(70, 23, false, 0, drift, strength);
        };
    }

    private record ChamberShape(double centerY, double radiusY,
                                boolean flooded, double waterline,
                                double verticalDrift, double strength) {
    }
}
