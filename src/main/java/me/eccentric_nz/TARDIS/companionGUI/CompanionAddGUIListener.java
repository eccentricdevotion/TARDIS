 ...
 * @author eccentric_nz
 */
public class CompanionAddGUIListener extends TARDISMenuListener {

    private final TARDIS plugin;

    public CompanionAddGUIListener(TARDIS plugin) {
        super(plugin);
        this.plugin = plugin;
    }
    ...
    public void onCompanionAddGUIClick(InventoryClickEvent event) {
        if (!(event.getInventory().getHolder(false) instanceof CompanionAddInventory)) {
            return;
        }
        ...
        switch (slot) {
            ...
            default -> {
                ...
                if (rs.resultSet()) {
                    ...
                    int id = tardis.getTardisId();
                    String comps = tardis.getCompanions();
                    String u = ComponentUtils.stripColour(is.getData(DataComponentTypes.LORE).lines().getFirst());
                    addCompanion(id, comps, u);
                    if (plugin.isWorldGuardOnServer() && plugin.getConfig().getBoolean("preferences.use_worldguard")) {
                        String[] data = tardis.getChunk().split(":");
                        addToRegion(data[0], tardis.getOwner(), ComponentUtils.stripColour(is.getData(DataComponentTypes.CUSTOM_NAME)));
        ...
        plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, () -> {
            ...
            if (rs.fromUUID(player.getUniqueId().toString())) {
                ...
                if (comps.equalsIgnoreCase("everyone")) {
                    items = new EveryoneCompanionInventory(plugin, player);
                } else {
                    items = new CompanionInventory(plugin, comps.split(":"));
                }
