...
public class TARDISSiegeCommand {
    ...
    public boolean setOption(CommandSender sender, String[] args) {
        if (args.length == 2 && args[1].equalsIgnoreCase("true") || args[1].equalsIgnoreCase("false")) {
            plugin.getConfig().set("siege.enabled", Boolean.valueOf(args[1].toLowerCase()));
            plugin.saveConfig();
            return true;
        }
        if (args.length < 3) {
        ...
        if (!siegeArgs.contains(args[1].toLowerCase())) {
            TARDISMessage.send(sender, "ARG_NOT_VALID");
            return true;
        }
        if (siegeBool.contains(args[1].toLowerCase())) {
            String tf = args[2].toLowerCase();
            if (!tf.equals("true") && !tf.equals("false")) {
                TARDISMessage.send(sender, "TRUE_FALSE");
                return true;
            }
            plugin.getConfig().set("siege." + args[1].toLowerCase(), Boolean.valueOf(tf));
        }
        if (!siegeBool.contains(args[1].toLowerCase())) {
            int val;
            try {
