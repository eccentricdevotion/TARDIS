		...
		if (args.length > 2) {
				...
				if (category != RecipeCategory.UNUSED && category != RecipeCategory.UNCRAFTABLE) {
					...
					for (RecipeItem item : RecipeItem.values()) {
						if (item.getCategory() == category) {
							sender.sendMessage("| " + TARDISStringUtils.capitalise(item.name()) + " | `/tardisrecipe " +
											   item.toTabCompletionString() + "` |");
						}
					}
			...
			if (plugin.checkTWA()) {
				for (Monster m : Monster.values()) {
					sender.sendMessage(m.toString() + "_HEAD(\"" + m.getName() + " Head\", Material." +
									   m.getMaterial().toString() + ", " + m.getCustomModelData() + "),");
				}
			}
		}
	}
