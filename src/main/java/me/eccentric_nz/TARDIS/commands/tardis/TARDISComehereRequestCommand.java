 ...
 * @author eccentric_nz
 */
class TARDISComehereRequestCommand {

    private final TARDIS plugin;

    TARDISComehereRequestCommand(TARDIS plugin) {
        this.plugin = plugin;
    }

    boolean requestComeHere(Player player, String[] args) {
        if (TARDISPermission.hasPermission(player, "tardis.timetravel")) {
            ...
            if (plugin.getConfig().getBoolean("allow.power_down") && !tardis.isPowered_on()) {
                TARDISMessage.send(player, "PLAYER_NOT_POWERED", requested.getDisplayName());
                return true;
            }
            ...
            if (TARDISPermission.hasPermission(requested, "tardis.exile") && plugin.getConfig().getBoolean("travel.exile")) {
                ...
                if (plugin.getTardisArea().areaCheckInExile(areaPerm, eyeLocation)) {
                    TARDISMessage.send(player, "EXILE_NO_REQUEST", requested.getDisplayName());
                    return true;
                }
            }
            if (plugin.getTrackerKeeper().getDispersedTARDII().contains(id)) {
                TARDISMessage.send(player.getPlayer(), "NOT_WHILE_DISPERSED_REQUEST", requested.getDisplayName());
                return true;
            }
            ...
            if (!rsti.resultSet()) {
                TARDISMessage.send(player, "PLAYER_NOT_IN_TARDIS", requested.getDisplayName());
                return true;
            }
            ...
            plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, () -> {
                if (plugin.getTrackerKeeper().getComehereRequests().containsKey(playerUuid)) {
                    ...
                    TARDISMessage.send(player, "REQUEST_NO_RESPONSE", requested.getDisplayName());
                }
            }, 1200L);
            return true;
        } else {
