 ...
 * @author eccentric_nz
 */
public class ChameleonConstructorListener extends TARDISMenuListener {
        ...
        if (slot >= 0 && (slot < 18 || slot == 26 || slot == 43 || slot == 52)) {
            ...
            switch (slot) {
                ...
                case 0 -> plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, () ->
                        player.openInventory(new ChameleonInventory(plugin, adapt, preset, tardis.getItemPreset()).getInventory()), 2L);
                // help
