                ...
                if (rsp.resultSet()) {
                    ...
                    if (rsp.isAutoOn() && !tardis.isSiegeOn() && !plugin.getTrackerKeeper().getDispersedTARDII().contains(id)) {
                        ...
                        if (tardis.getArtronLevel() > amount) {
                            ...
                            if (goto_loc != null && !AutonomousUtils.compareCurrentToHome(current.location(), rsh)) {
                                ...
                                if (!plugin.getTrackerKeeper().getDestinationVortex().containsKey(id)) {
                                    ...
                                    if (!tardis.isHidden()) {
                                        ...
                                        CloisterBell bell = new CloisterBell(plugin, 5, id, current.location(), plugin.getServer().getPlayer(uuid), true, "Time Lord death", false);
                                ...
                                plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, () -> {
                                    ...
                                    // sound the cloister bell at current location for dematerialisation
                                    CloisterBell bell = new CloisterBell(plugin, 6, id, current.location(), plugin.getServer().getPlayer(uuid), false, "", true);
                                    int taskID = plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, bell, 2L, 70L);
                                    bell.setTask(taskID);
                                    plugin.getTrackerKeeper().getCloisterBells().put(id, taskID);
                                    // set handbrake on
                            ...
                            CloisterBell bell = new CloisterBell(plugin, 7, id, current.location(), plugin.getServer().getPlayer(uuid), true, "Siege mode engaged", false);
                            int taskID = plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, bell, 2L, 70L);
                            bell.setTask(taskID);
                            plugin.getTrackerKeeper().getCloisterBells().put(id, taskID);
                            // place siege block
                            ...
                            if (plugin.getConfig().getBoolean("siege.texture")) {
                                ...
                                // start the rebuild
                                WallFloorRunnable ttr = new WallFloorRunnable(plugin, player.getUniqueId(), tud);
