    ...
    public void run() {
        ...
        if (level == (h - 1) && row == (w - 1)) {
            ...
            lampblocks.forEach((lamp) -> {
                Material l = (tud.getSchematic().hasLanterns() || (archive != null && archive.isLanterns())) ? Material.SEA_LANTERN : Material.REDSTONE_LAMP_ON;
                lamp.setType(l);
            });
            lampblocks.clear();
            if (postBedrock != null) {
            ...
            for (int col = 0; col < c; col++) {
                ...
                if (type.equals(Material.IRON_DOOR_BLOCK)) { // doors
                    ...
                    if (clean && !tmp.getType().equals(type) && !tmp.getType().equals(Material.AIR)) {
