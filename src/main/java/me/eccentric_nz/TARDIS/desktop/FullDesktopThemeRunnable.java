 ...
 * @author eccentric_nz
 */
public class FullDesktopThemeRunnable extends DesktopThemeRunnable {
    ...
    public void run() {
        ...
        if (!running) {
            ...
            if (archive_next == null) {
                // get JSON
                obj = SchematicGZip.getObject(plugin, "consoles", tud.getSchematic().getPermission(), tud.getSchematic().isCustom());
                if (obj == null) {
                    // cancel task
                    plugin.getServer().getScheduler().cancelTask(taskID);
                    return;
            ...
            if (archive_prev == null) {
                // get JSON
                JsonObject prevObj = SchematicGZip.getObject(plugin, "consoles", tud.getPrevious().getPermission(), tud.getPrevious().isCustom());
                if (prevObj == null) {
                    // cancel task
                    plugin.getServer().getScheduler().cancelTask(taskID);
                    return;
            ...
            if (tardis.getRotor() != null) {
                ...
                TimeRotor.updateRotorRecord(id, "");
            ...
            chunks = ChunkUtils.getConsoleChunks(chunk, tud.getSchematic());
            previousChunks = ChunkUtils.getConsoleChunks(chunk, tud.getPrevious());
        ...
        if (level == (height - 1) && row == (width - 1)) {
            ...
            for (int col = 0; col < c; col++) {
                ...
                if (type.equals(Material.SCULK_SHRIEKER)) {
                    ...
                    TimeRotor.updateRotorRecord(id, shrieker);
                }
