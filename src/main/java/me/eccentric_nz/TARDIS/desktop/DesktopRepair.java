 ...
 * @author eccentric_nz
 */
public class DesktopRepair {

    private final TARDIS plugin;
    private final Player player;

    public DesktopRepair(TARDIS plugin, Player player) {
        this.plugin = plugin;
        this.player = player;
    }
    ...
    public void restore(boolean clean) {
        ...
        if (rs.resultSet()) {
            ...
            DesktopThemeRunnable ttr = new RepairDesktopRunnable(plugin, uuid, tud, clean);
            // start the rebuild
    ...
    private JsonObject getConsole() {
        ...
        if (rs.resultSet()) {
            ...
            if (perm.equals("archive")) {
                ...
                // get JSON
                obj = SchematicGZip.getObject(plugin, "consoles", tardis.getSchematic().getPermission(), tardis.getSchematic().isCustom());
            }
        }
        return obj;
    }
