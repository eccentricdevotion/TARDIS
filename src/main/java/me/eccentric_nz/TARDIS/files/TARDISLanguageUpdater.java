...
public class TARDISLanguageUpdater {
    ...
    public void update() {
        ...
        if (i > 0) {
            plugin.getMessenger().message(plugin.getConsole(), TardisModule.TARDIS, "Added " + i + " new messages to " + lang + ".yml");
        ...
        if (j > 0) {
            plugin.getMessenger().message(plugin.getConsole(), TardisModule.TARDIS, "Removed " + j + " redundant messages from " + lang + ".yml");
        }
    }
}
