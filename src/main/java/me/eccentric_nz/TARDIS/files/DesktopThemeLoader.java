 ...
 * @author eccentric_nz
 */
public class DesktopThemeLoader {

    private final TARDIS plugin;
    private boolean save = false;

    public DesktopThemeLoader(TARDIS plugin) {
        this.plugin = plugin;
    }
    ...
    public void addSchematics() {
        ...
        Desktops.getBY_NAMES().put("DINER", new Schematic("STRIPPED_CRIMSON_HYPHAE", "diner", "Clara's diner", ConsoleSize.MEDIUM, false, TardisLight.LANTERN, false, -62));
        ...
        for (String desktop : plugin.getCustomDesktopsConfig().getKeys(false)) {
            if (plugin.getCustomDesktopsConfig().getBoolean(desktop + ".enabled")) {
                ...
                // get JSON
                JsonObject obj = SchematicGZip.unzip(path);
                // get dimensions
