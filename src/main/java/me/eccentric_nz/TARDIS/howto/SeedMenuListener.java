 ...
 * @author eccentric_nz
 */
public class SeedMenuListener extends TARDISMenuListener {

    private final TARDIS plugin;

    public SeedMenuListener(TARDIS plugin) {
        super(plugin);
        this.plugin = plugin;
    }
    ...
    public void onSeedMenuClick(InventoryClickEvent event) {
        if (!(event.getInventory().getHolder(false) instanceof SeedsInventory)) {
            return;
        }
        ...
        if (is != null) {
            ...
            plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, () ->
                    p.openInventory(new SeedRecipeInventory(plugin, is.getType()).getInventory()), 2L);
        }
    }
