...
import me.eccentric_nz.TARDIS.transmat.TransmatInventory;
 ...
 * @author eccentric_nz
 */
public class ControlMenuListener extends TARDISMenuListener {

    private final TARDIS plugin;

    public ControlMenuListener(TARDIS plugin) {
        super(plugin);
        this.plugin = plugin;
    }
    ...
    public void onControlMenuInteract(InventoryClickEvent event) {
        ...
        if (!(inventory.getHolder(false) instanceof ControlInventory)) {
        ...
        switch (slot) {
            ...
            case 4 -> {
                ...
                player.openInventory(new ChameleonInventory(plugin, tardis.getAdaption(), tardis.getPreset(), tardis.getItemPreset()).getInventory());
            }
            case 6 -> {
                // artron level
                close(player, true);
            ...
            case 26 -> {
                ...
                if (comps == null || comps.isEmpty()) {
                    ...
                    plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, () ->
                            player.openInventory(new CompanionAddInventory(plugin, player).getInventory()), 2L);
                    return;
                }
            ...
            case 29 -> player.openInventory(new LightsInventory(plugin, id, player.getUniqueId()).getInventory());
            case 31 -> {
                // rebuild
                close(player, true);
                if (plugin.getTrackerKeeper().getInSiegeMode().contains(id)) {
            ...
            // transmat
            case 33 -> player.openInventory(new TransmatInventory(plugin, id, player).getInventory());
