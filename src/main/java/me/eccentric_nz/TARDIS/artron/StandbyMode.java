 ...
 * @author eccentric_nz
 */
public class StandbyMode implements Runnable {

    private final TARDIS plugin;
    private final int amount;

    public StandbyMode(TARDIS plugin) {
        this.plugin = plugin;
            ...
            if (!isTravelling(id) && !isNearCharger(id) && level > amount) {
                ...
                plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, () -> {
                    ...
                    if (standbyData.preset().equals(ChameleonPreset.ADAPTIVE) || standbyData.preset().usesArmourStand()) {
                        plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, () -> new AdaptiveBoxLampToggler(plugin).toggleLamp(id, false, standbyData.preset()), delay);
                    }
