...
public class SonicRedstone {
    ...
    public static void togglePoweredState(TARDIS plugin, Player player, Block block) {
        ...
        if (SonicRespect.checkBlockRespect(plugin, player, block)) {
            SonicSound.playSonicSound(plugin, player, System.currentTimeMillis(), 600L, "sonic_short");
            ...
            switch (blockType) {
                ...
                case IRON_DOOR -> {
                    ...
                    // not TARDIS doors!
                    String doorloc = tmp.getLocation().getWorld().getKey().asString() + ":" + tmp.getLocation().getBlockX() + ":" + tmp.getLocation().getBlockY() + ":" + tmp.getLocation().getBlockZ();
