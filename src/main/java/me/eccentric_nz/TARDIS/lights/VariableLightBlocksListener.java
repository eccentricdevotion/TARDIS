...
public class VariableLightBlocksListener extends WallListener {
    ...
    public void onVariableLightBlockMenuOpen(InventoryOpenEvent event) {
        if (!(event.getInventory().getHolder(false) instanceof WallsInventory lights)) {
            return;
        }
    ...
    private void setVariableLightBlock(Player player, String block) {
        ...
        if (rst.fromUUID(uuid.toString())) {
            ...
            plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, () -> {
                ...
                player.openInventory(new LightsInventory(plugin, id, uuid).getInventory());
            }, 5L);
        }
    }
}
