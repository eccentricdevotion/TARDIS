...
public class SchematicFlowers {
    private final List<Material> smallFlowers;
    private final Material from = Material.SHORT_GRASS;

    public SchematicFlowers() {
    ...
    public void act(TARDIS plugin, Player player) {
            ...
            for (int r = minx; r <= maxx; r++) {
                for (int c = minz; c <= maxz; c++) {
                    ...
                    if (b.getType().equals(from)) {
                        Material to = smallFlowers.get(TARDISConstants.RANDOM.nextInt(smallFlowers.size()));
                        b.setType(to);
                    }
                }
            }
