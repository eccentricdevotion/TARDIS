 ...
 * @author eccentric_nz
 */
class HostileDisplacement {

    private final TARDIS plugin;
    private final List<Integer> angles;
    private int count = 0;

    HostileDisplacement(TARDIS plugin) {
            ...
            if (bool) {
                ...
                if (safe) {
                    ...
                    if (plugin.getPluginRespect().getRespect(fl, new Parameters(player, Flag.getNoMessageFlags()))) {
                        ...
                        CloisterBell bell = new CloisterBell(plugin, 5, id, current.location(), plugin.getServer().getPlayer(uuid), true, "HADS displacement", false);
                        ...
                        plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, () -> {
                            CloisterBell end = new CloisterBell(plugin, 6, id, fl, plugin.getServer().getPlayer(uuid), false, "", true);
                    ...
                    new HostileDispersal(plugin).disperseTARDIS(id, uuid, hostile, preset);
                }
            } else {
