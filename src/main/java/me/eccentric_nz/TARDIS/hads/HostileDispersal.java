...
class HostileDispersal {
    ...
    void disperseTARDIS(int id, UUID uuid, Player hostile, ChameleonPreset preset) {
        ...
        CloisterBell bell = new CloisterBell(plugin, 3, id, current.location(), plugin.getServer().getPlayer(uuid), "HADS dispersal");
            ...
            for (int xx = 0; xx < 3; xx++) {
                for (int zz = 0; zz < 3; zz++) {
                    ...
                    if (!b.getType().isAir()) {
                        ...
                        String stainedGlass = StainedGlassLookup.stainedGlassFromMaterial(b.getWorld(), b.getType()).toString();
