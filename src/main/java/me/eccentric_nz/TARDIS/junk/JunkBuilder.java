 ...
 * @author eccentric_nz
 */
public class JunkBuilder implements Runnable {
    ...
    public JunkBuilder(TARDIS plugin, BuildData bd) {
        ...
        if (!plugin.getTrackerKeeper().getDematerialising().contains(bd.getTardisID())) {
            ...
            if (i < 24) {
                ...
                if (i == 2) {
                    ...
                    fryTask = plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, new JunkItsDangerousRunnable(plugin, loc), 0, 1L);
                }
                if (i == 1) {
                    ...
                    // get JSON
                    JsonObject obj = SchematicGZip.getObject(plugin, "consoles", "junk", false);
                    if (obj != null) {
                        // get dimensions
                ...
                new JunkReturnPersiter(plugin).setJunkTime();
            }
        }
    }
