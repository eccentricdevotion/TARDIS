 ...
 * @author eccentric_nz
 */
class JunkVortexRunnable implements Runnable {
    ...
    public void run() {
        if (i < LOOPS) {
            i++;
            if (i == 1) {
                fryTask = plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, new JunkItsDangerousRunnable(plugin, vortexJunkLoc), 0, 1L);
            }
