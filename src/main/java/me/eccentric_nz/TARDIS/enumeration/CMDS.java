...
public enum CMDS {
    ...
    CREATE(ChatColor.AQUA + "Creating a TARDIS" + ChatColor.RESET + " You create a TARDIS by placing a " + ChatColor.GOLD + "specific pattern of blocks." + ChatColor.RESET + " You will need to have an IRON, GOLD, REDSTONE, EMERALD or DIAMOND BLOCK, a LAPIS BLOCK, and a redstone torch in your inventory. You place the blocks where you want the TARDIS to be, in the following order: Bottom - IRON, GOLD, REDSTONE, EMERALD or DIAMOND BLOCK, middle - LAPIS BLOCK, top - REDSTONE TORCH The TARDIS takes up a 3 x 3 x 4 area (w x d x h), so keep this in mind. To enter the TARDIS, right-click the door with your TARDIS key (by default a GOLD_NUGGET)."),
