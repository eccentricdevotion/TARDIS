 ...
 * @author eccentric_nz
 */
class DematerialiseBlockPreset implements Runnable {
     ...
     */
    DematerialiseBlockPreset(TARDIS plugin, DestroyData dd, ChameleonPreset preset, BlockData cham_id) {
        this.plugin = plugin;
        this.dd = dd;
        ...
        if (this.preset.equals(ChameleonPreset.CONSTRUCT)) {
            column = new ConstructColumn(plugin, dd.getTardisID(), "blueprintData", dd.getDirection().forPreset()).getColumn();
            stained_column = new ConstructColumn(plugin, dd.getTardisID(), "stainData", dd.getDirection().forPreset()).getColumn();
            glass_column = new ConstructColumn(plugin, dd.getTardisID(), "glassData", dd.getDirection().forPreset()).getColumn();
    ...
    public void run() {
        ...
        if (i < loops) {
                    ...
                    for (int yy = 3; yy >= 0; yy--) {
                        ...
                        switch (mat) {
                            ...
                            case LIGHT_GRAY_STAINED_GLASS -> {
                                BlockData cham = (preset.equals(ChameleonPreset.FACTORY)) ? StainedGlassLookup.stainedGlassFromMaterial(world, cham_id.getMaterial()).createBlockData() : colData[yy];
            ...
            plugin.getServer().getScheduler().cancelTask(task);
            task = 0;
            new InstantPresetDestroyer(plugin).instaDestroyPreset(dd, dd.isHide(), preset);
            if (preset.equals(ChameleonPreset.JUNK_MODE)) {
