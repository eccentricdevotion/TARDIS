        ...
        if (rs.resultSet()) {
            ...
            if (!demat.equals(ChameleonPreset.INVISIBLE)) {
                ...
                if (demat.equals(ChameleonPreset.JUNK)) {
                    JunkDestroyer runnable = new JunkDestroyer(plugin, dd);
                    ...
                    if (demat.usesArmourStand()) {
                        DematerialiseModelledPreset frame = new DematerialiseModelledPreset(plugin, dd, demat);
                        taskID = plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, frame, 10L, 20L);
                        frame.setTask(taskID);
                    } else {
                        DematerialiseBlockPreset runnable = new DematerialiseBlockPreset(plugin, dd, demat, cham_id.createBlockData());
                        taskID = plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, runnable, 10L, 20L);
                        runnable.setTask(taskID);
                    }
                }
            } else {
                new InstantPresetDestroyer(plugin).instaDestroyPreset(dd, dd.isHide(), demat);
            }
        }
    }
