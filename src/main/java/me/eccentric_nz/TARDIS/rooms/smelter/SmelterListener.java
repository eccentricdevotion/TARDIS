...
public class SmelterListener implements Listener {
    ...
    public SmelterListener(TARDIS plugin) {
        this.plugin = plugin;
    }
    ...
    public void onSmelterDropChestClose(InventoryCloseEvent event) {
        ...
        if (holder instanceof Chest chest) {
            ...
            switch (rs.getType()) {
                case FUEL -> new SmelterFuel().processItems(inv, fuelChests);
                case SMELT -> new SmelterOre().processItems(inv, oreChests);
                // DROP
                default -> new SmelterDrop().processItems(inv, fuelChests, oreChests);
            }
        }
    }
}
