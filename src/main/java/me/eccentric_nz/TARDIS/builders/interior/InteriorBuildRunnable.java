 ...
 * @author eccentric_nz
 */
public class InteriorBuildRunnable implements Runnable {
    ...
    public void run() {
        if (!running) {
            ...
            // get JSON
            obj = SchematicGZip.getObject(plugin, "consoles", schm.getPermission(), schm.isCustom());
            if (obj != null) {
                // get dimensions
                ...
                chunkList = ChunkUtils.getConsoleChunks(world, wg1.getChunk().getX(), wg1.getChunk().getZ(), w, d);
        ...
        for (int col = 0; col <= d; col++) {
            ...
            if (type.equals(Material.SCULK_SHRIEKER)) {
                ...
                TimeRotor.updateRotorRecord(dbID, shrieker);
            }
