    ...
    public void buildPreset(BuildData bd) {
        ...
        if (rs.resultSet()) {
            ...
            if (bd.isRebuild()) {
                ...
                if (!hidden) {
                    InstantPresetDestroyer deinsta = new InstantPresetDestroyer(plugin);
                ...
                if (preset.usesArmourStand()) {
                    MaterialiseModelledPreset stand = new MaterialiseModelledPreset(plugin, bd, preset);
                    taskID = plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, stand, 10L, 20L);
                    stand.setTask(taskID);
                } else {
                    MaterialiseBlockPreset runnable = new MaterialiseBlockPreset(plugin, bd, preset, chameleonMaterial.createBlockData(), tardis.getAdaption());
                ...
                if (preset.equals(ChameleonPreset.JUNK)) {
                    JunkBuilder runnable = new JunkBuilder(plugin, bd);
                ...
                plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, () -> {
                    plugin.getTrackerKeeper().getMaterialising().add(bd.getTardisID());
                    new InstantBlockPresetBuilder(plugin, bd, ChameleonPreset.INVISIBLE, material.createBlockData(), false).buildPreset();
                }, 375L);
            }
