 ...
 * @author eccentric_nz
 */
public class MaterialseFromVortex implements Runnable {
    ...
    public MaterialseFromVortex(TARDIS plugin, int id, Player player, Location handbrake, Throticle throticle) {
        this.plugin = plugin;
        this.id = id;
        this.player = player;
            ...
            if (rsc.resultSet()) {
                ...
                if (malfunction) {
                    ...
                    if (exit != null) {
                        ...
                        scheduler.scheduleSyncDelayedTask(plugin, () -> {
                            ...
                            CloisterBell bell = new CloisterBell(plugin, 6, id, location, plugin.getServer().getPlayer(uuid), true, "a flight malfunction", false);
                ...
                if (exit != null) {
                    ...
                    if (flight_mode == 2 || flight_mode == 3) {
                        ...
                        Runnable runner;
                        if (flight_mode == 2) {
                            runner = new RegulatorStarter(plugin, player, id);
                        } else {
                            ResultSetInteractionCheck rsic = new ResultSetInteractionCheck(plugin);
                            runner = new ManualFlightStarter(plugin, player, id, rsic.resultSetFromId(id));
                        }
                    ...
                    if (bar) {
                        long tt = travel_time;
                        // start travel bar
                        scheduler.scheduleSyncDelayedTask(plugin, () -> new TravelBar(plugin, id).showTravelRemaining(player, tt, false), flight_mode_delay);
                    }
