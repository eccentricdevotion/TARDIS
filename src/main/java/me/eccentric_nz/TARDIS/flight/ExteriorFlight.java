 ...
 * @author eccentric_nz
 */
public class ExteriorFlight {

    private final TARDIS plugin;

    public ExteriorFlight(TARDIS plugin) {
        this.plugin = plugin;
    }
    ...
    public void stopFlying(Player player, ArmorStand stand) {
        ...
        if (data != null) {
            ...
            if (!plugin.getPluginRespect().getRespect(location, new Parameters(player, Flag.getDefaultFlags())) || plugin.getTardisArea().isInExistingArea(location)) {
                ...
                plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, new LoopingFlightSound(plugin, interior, data.id()), 20L);
