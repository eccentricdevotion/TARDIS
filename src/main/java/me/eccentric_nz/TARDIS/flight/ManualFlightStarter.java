 ...
 * @author eccentric_nz
 */
class ManualFlightStarter implements Runnable {
    ...
    ManualFlightStarter(TARDIS plugin, Player player, int id, boolean console) {
        this.plugin = plugin;
        this.player = player;
        this.id = id;
    ...
    public void run() {
        ...
        ManualFlightRunnable mfr = new ManualFlightRunnable(plugin, player, id, console);
        int taskid = plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, mfr, 10L, delay);
        mfr.setTaskID(taskid);
        // play inflight sound
        if (!plugin.getTrackerKeeper().getDestinationVortex().containsKey(id)) {
            plugin.getServer().getScheduler().runTask(plugin, new LoopingFlightSound(plugin, player.getLocation(), id));
        }
    }
}
