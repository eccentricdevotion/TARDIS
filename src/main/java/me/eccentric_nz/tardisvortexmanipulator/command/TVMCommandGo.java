...
public class TVMCommandGo {
    ...
    public void execute(Player player, String save) {
        ...
        // check save exists
        TVMResultSetWarpByName rsw = new TVMResultSetWarpByName(plugin, uuid, save);
        if (!rsw.resultSet()) {
            plugin.getMessenger().send(player, TardisModule.VORTEX_MANIPULATOR, "VM_NO_SAVE");
            return;
        }
        Location l = rsw.getWarp();
        plugin.getMessenger().send(player, TardisModule.VORTEX_MANIPULATOR, "VM_STANDBY_TO", save);
