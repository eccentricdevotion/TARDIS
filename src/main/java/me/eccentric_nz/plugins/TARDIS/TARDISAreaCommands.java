        ...
        if (cmd.getName().equalsIgnoreCase("tardisarea")) {
            ...
            if (args[0].equals("start")) {
                ...
                if (args.length < 2 || !args[1].matches("[A-Za-z0-9_]{2,16}")) {
                    sender.sendMessage(plugin.MY_PLUGIN_NAME + "That doesn't appear to be a valid area name (it may be too long)" + ChatColor.GREEN + " /tardis admin area start [area_name_goes_here]");
                    return false;
                }
                ...
                try {
                    ...
                    while (rsName.next()) {
                        if (rsName.getString("area_name").equals(args[1])) {
                            sender.sendMessage(plugin.MY_PLUGIN_NAME + " Area name already in use!");
                            return false;
                        }
                    }
                } catch (SQLException e) {
                    plugin.console.sendMessage(plugin.MY_PLUGIN_NAME + "Couldn't get area names: " + e);
                } finally {
                    if (rsName != null) {
                        try {
                            rsName.close();
                ...
                player.sendMessage(plugin.MY_PLUGIN_NAME + " Click the area start block to save its position.");
                return true;
            }
            if (args[0].equals("end")) {
                if (!plugin.trackBlock.containsKey(player.getName())) {
                    player.sendMessage(plugin.MY_PLUGIN_NAME + ChatColor.RED + " You haven't selected an area start block!");
                    return false;
                }
                plugin.trackEnd.put(player.getName(), "end");
                player.sendMessage(plugin.MY_PLUGIN_NAME + " Click the area end block to complete the area.");
                return true;
            }
            if (args[0].equals("remove")) {
                ...
                try {
                    ...
                    player.sendMessage(plugin.MY_PLUGIN_NAME + " Area [" + args[1] + "] deleted!");
                    return true;
                } catch (SQLException e) {
                    plugin.console.sendMessage(plugin.MY_PLUGIN_NAME + "Couldn't delete area: " + e);
                } finally {
                    try {
                        statement.close();
                    } catch (Exception e) {
            ...
            if (args[0].equals("show")) {
                ...
                try {
                    ...
                    if (!rsArea.next()) {
                        player.sendMessage(plugin.MY_PLUGIN_NAME + "Could not find area [" + args[1] + "]! Did you type the name correctly?");
                        return false;
                    }
