    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cmd.getName().equalsIgnoreCase("tardisprefs")) {
            ...
            if (args[0].equalsIgnoreCase("sfx")) {
                if (player.hasPermission("tardis.timetravel")) {
                    if (args.length < 2 || (!args[1].equalsIgnoreCase("on") && !args[1].equalsIgnoreCase("off"))) {
                        sender.sendMessage(plugin.MY_PLUGIN_NAME + " You need to specify if sound effects should be on or off!");
                        return false;
                    }
                    ...
                    try {
                        ...
                        if (args[1].equalsIgnoreCase("on")) {
                            ...
                            sender.sendMessage(plugin.MY_PLUGIN_NAME + " Sound effects were turned ON!");
                        }
                        if (args[1].equalsIgnoreCase("off")) {
                            String queryUpdate = "UPDATE player_prefs SET sfx_on = 0 WHERE player = '" + player.getName() + "'";
                            statement.executeUpdate(queryUpdate);
                            sender.sendMessage(plugin.MY_PLUGIN_NAME + " Sound effects were turned OFF.");
                        }
                        return true;
                    } catch (SQLException e) {
                        plugin.console.sendMessage(plugin.MY_PLUGIN_NAME + " SFX Preferences Save Error: " + e);
                    } finally {
                        if (rs != null) {
                            try {
                                rs.close();
                ...
                if (player.hasPermission("tardis.timetravel")) {
                    ...
                    try {
                        ...
                        if (args[1].equalsIgnoreCase("on")) {
                            ...
                            statement.executeUpdate(queryUpdate);
                            sender.sendMessage(plugin.MY_PLUGIN_NAME + " The safety platform was turned ON!");
                        }
                        if (args[1].equalsIgnoreCase("off")) {
                            String queryUpdate = "UPDATE player_prefs SET platform_on = 0 WHERE player = '" + player.getName() + "'";
                            statement.executeUpdate(queryUpdate);
                            sender.sendMessage(plugin.MY_PLUGIN_NAME + " safety platform was turned OFF.");
                        }
                        return true;
                    } catch (SQLException e) {
                        plugin.console.sendMessage(plugin.MY_PLUGIN_NAME + " Platform Preferences Save Error: " + e);
                    } finally {
                        if (rs != null) {
                            try {
                                rs.close();
            ...
            if (args[0].equalsIgnoreCase("quotes")) {
                if (player.hasPermission("tardis.timetravel")) {
                    if (args.length < 2 || (!args[1].equalsIgnoreCase("on") && !args[1].equalsIgnoreCase("off"))) {
                        sender.sendMessage(plugin.MY_PLUGIN_NAME + " You need to specify if Who quotes should be on or off!");
                        return false;
                    }
                    ...
                    try {
                        ...
                        if (args[1].equalsIgnoreCase("on")) {
                            ...
                            statement.executeUpdate(queryUpdate);
                            sender.sendMessage(plugin.MY_PLUGIN_NAME + " Quotes were turned ON!");
                        }
                        if (args[1].equalsIgnoreCase("off")) {
                            String queryUpdate = "UPDATE player_prefs SET quotes_on = 0 WHERE player = '" + player.getName() + "'";
                            statement.executeUpdate(queryUpdate);
                            sender.sendMessage(plugin.MY_PLUGIN_NAME + " Quotes were turned OFF.");
                        }
                        return true;
                    } catch (SQLException e) {
