...
public class TARDISConfiguration {
    ...
    public void checkConfig() {
        ...
        if (i > 0) {
            plugin.console.sendMessage(plugin.MY_PLUGIN_NAME + " Added '" + ChatColor.AQUA + i + ChatColor.RESET + "' new items to config");
        }
        // add worlds
        ...
        for (World w : worlds) {
            ...
            if (!config.contains(worldname)) {
                ...
                plugin.console.sendMessage(plugin.MY_PLUGIN_NAME + " Added '" + w.getName() + "' to config. To exclude this world run: /tardis admin exclude " + w.getName());
            }
        }
        plugin.saveConfig();
        ...
        for (String cw : cWorlds) {
            if (plugin.getServer().getWorld(cw) == null) {
                ...
                plugin.console.sendMessage(plugin.MY_PLUGIN_NAME + " Removed '" + cw + " from config.yml");
            }
        }
        plugin.saveConfig();
    }
