 ...
 */
package me.eccentric_nz.tardisweepingangels.monsters.clockwork_droids;
...
public class ClockworkDroidRunnable implements Runnable {

    private final TARDIS plugin;
    private final int spawn_rate;

    public ClockworkDroidRunnable(TARDIS plugin) {
        this.plugin = plugin;
    ...
    public void run() {
        plugin.getServer().getWorlds().forEach((w) -> {
            ...
            if (plugin.getMonstersConfig().getInt("clockwork_droids.worlds." + name) > 0) {
                ...
                if (clockwork_droids < plugin.getMonstersConfig().getInt("clockwork_droids.worlds." + name)) {
                    // if less than maximum, spawn some more
                    for (int i = 0; i < spawn_rate; i++) {
                        spawnCLockworkDroid(w);
                    }
                }
            }
        });
    ...
    private void spawnCLockworkDroid(World world) {
