...
public class HathRunnable implements Runnable {
    ...
    public HathRunnable(TARDIS plugin) {
        this.plugin = plugin;
    ...
    public void run() {
        plugin.getServer().getWorlds().forEach((w) -> {
            ...
            if (plugin.getMonstersConfig().getInt("hath.worlds." + name) > 0) {
                ...
                if (hath < plugin.getMonstersConfig().getInt("hath.worlds." + name)) {
    ...
    private void spawnHath(World world) {
        ...
        if (chunks.length > 0) {
            ...
            if (WaterChecker.isNotWater(l)) {
                ...
                LivingEntity hath = new MonsterSpawner().create(l, Monster.HATH, null);
