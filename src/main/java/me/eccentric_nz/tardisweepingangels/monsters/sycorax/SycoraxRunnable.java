 ...
 */
package me.eccentric_nz.tardisweepingangels.monsters.sycorax;
...
public class SycoraxRunnable implements Runnable {

    private final TARDIS plugin;
    private final int spawn_rate;

    public SycoraxRunnable(TARDIS plugin) {
        this.plugin = plugin;
    ...
    public void run() {
        plugin.getServer().getWorlds().forEach((w) -> {
            ...
            if (plugin.getMonstersConfig().getInt("sycorax.worlds." + name) > 0) {
                ...
                if (sycorax < plugin.getMonstersConfig().getInt("sycorax.worlds." + name)) {
    ...
    private void spawnSycorax(World world) {
