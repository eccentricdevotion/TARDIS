...
public class ZygonRunnable implements Runnable {
    ...
    public ZygonRunnable(TARDIS plugin) {
        this.plugin = plugin;
    ...
    public void run() {
        plugin.getServer().getWorlds().forEach((w) -> {
            ...
            if (plugin.getMonstersConfig().getInt("zygons.worlds." + name) > 0) {
                ...
                if (zygons < plugin.getMonstersConfig().getInt("zygons.worlds." + name)) {
...
//                LivingEntity zygon = (LivingEntity) world.spawnEntity(l, EntityType.ZOMBIE);
