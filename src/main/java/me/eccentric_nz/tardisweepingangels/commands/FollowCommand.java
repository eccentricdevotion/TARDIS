    ...
    public boolean follow(CommandSender sender, String[] args) {
        if (sender instanceof Player player) {
            if (TARDISWeepingAngels.getFollowTasks().containsKey(player.getUniqueId())) {
                player.sendMessage(plugin.getPluginName() + "An entity is already following you!");
                return true;
            }
            // get the armour stand
            ArmorStand stand = ArmourStandFinder.getStand(player);
            if (stand == null) {
                player.sendMessage(plugin.getPluginName() + "You are not looking at an entity that can follow you!");
                return true;
            }
